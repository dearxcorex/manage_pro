...
def start():
    while True:
        try:
            choice = int(input(
                "Enter 1 to select pro or 2 to run all pro or 3 for automation OPER[1,2,3] : "))
...
def handle_select_pro():
    ...
    select_pro(input_date, input_pro, input_percent_occunpancy_min,
               input_percent_occunpancy_max)
...
def all_pro(date,):
    ...
    run.pro_1(current_date.strftime(date_format),
              0, 0, freq=found_freq["pro1"])
    current_date += timedelta(days=1)

    run.pro_2(current_date.strftime(date_format),
              20, 55, freq=found_freq["pro2"])

    current_date += timedelta(days=1)
    run.pro_3(current_date.strftime(date_format),
              20, 55, freq=found_freq["pro3"])

    current_date += timedelta(days=1)
    run.pro_4(current_date.strftime(date_format),
              30, 45, freq=found_freq["pro4"])

    current_date += timedelta(days=1)
    run.pro_5(current_date.strftime(date_format),
              30, 45, freq=found_freq["pro5"])

    current_date += timedelta(days=1)
    run.pro_6(current_date.strftime(date_format),
              45, 55, freq=found_freq["pro6"])

    current_date += timedelta(days=1)
    run.pro_7(current_date.strftime(date_format),
              0, 0, freq=found_freq["pro7"])

    current_date += timedelta(days=1)
    run.pro_8(current_date.strftime(date_format),
              0, 0, freq=found_freq["pro8"])

    current_date += timedelta(days=1)
    run.pro_9(current_date.strftime(date_format),
              0, 0, freq=found_freq["pro9"])

    current_date += timedelta(days=1)
    run.pro_10(current_date.strftime(date_format),
               0, 0, freq=found_freq["pro10"])

    current_date += timedelta(days=1)
    run.pro_11(current_date.strftime(date_format),
               0, 0, freq=found_freq["pro11"])
